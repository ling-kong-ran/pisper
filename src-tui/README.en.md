...
The bottom status bar is left-aligned. While a session is running, a colorful energy wave appears at the far left, with its peak traveling back and forth across a fixed-width set of bars, for example `▁▂▄▆█▆▄`, instead of phase labels such as `Thinking` or `Responding`. Execution mode, model, thinking level, and metrics follow, for example `▁▂▄▆█▆▄  [full-access]  gpt-5.6-sol  high  ·  88M  ·  cache 79%`. Queue and approval positions follow the metrics. The values remain isolated per conversation.
