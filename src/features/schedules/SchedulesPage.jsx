...
import { AlertTriangle, Bell, Bot, CheckCircle2, ChevronDown, FolderOpen, MessageCircle, Play, Plus, RefreshCw, Trash2, X } from 'lucide-react'
...
const SCHEDULE_EXECUTION_MODES = ['full-access', 'workspace', 'read-only']

function ScheduleExecutionModeField({ value, onChange }) {
  const { t } = useI18n()
  const help = value === 'full-access'
    ? '后台运行不使用本地沙箱，适合需要网络或外部目录的任务。'
    : value === 'workspace'
      ? 'Shell 在工作区沙箱中运行；受阻时后台任务无法等待交互授权。'
      : '仅允许读取和分析，不允许修改文件或运行 Shell。'
  return <label className="field-label">{t('执行模式')}<span className="select-wrap"><select value={value} onChange={(event) => onChange(event.target.value)}>{SCHEDULE_EXECUTION_MODES.map((mode) => <option value={mode} key={mode}>{t(mode === 'full-access' ? '完全访问' : mode === 'workspace' ? '工作区' : '只读')}</option>)}</select><ChevronDown size={13} /></span><small>{t(help)}</small></label>
}

function ScheduleWorkspaceField({ value, name, onChange }) {
  const { t } = useI18n()
  const [open, setOpen] = useState(false)
  return <>
    <label className="field-label">{t('工作目录')}<span className="schedule-workspace-input"><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={t('输入项目的绝对路径')} /><button type="button" className="button secondary" onClick={() => setOpen(true)}><FolderOpen size={13} />{t('浏览目录')}</button></span><small>{t('定时任务的 Agent 将在此目录中运行。')}</small></label>
    {open && <WorkspacePicker session={{ name: name || t('定时任务'), cwd: value }} onClose={() => setOpen(false)} onSelect={(cwd) => { onChange(cwd); setOpen(false) }} />}
  </>
}
...
export function SchedulesPage({ notify, registerPrimaryAction, openNotificationSettings, requestConfirm }) {
  ...
  const [data, setData] = useState({ tasks: [], runs: [], notificationTargets: {}, defaultCwd: '' })
...
function CreateSchedulePanel({ notificationTargets, defaultCwd, onCreated }) {
  ...
  const [executionMode, setExecutionMode] = useState('full-access')
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const create = async () => {
    setSaving(true); setError('')
...
function CreateScheduleModal({ notificationTargets, defaultCwd, onClose, onCreated }) {
  ...
  const submit = async (event) => {
    ...
    try {
      const notifications = Object.entries(notificationTargets).filter(([, value]) => value.enabled).map(([id]) => id)
      onCreated(await apiJson('/api/schedules', { method: 'POST', body: JSON.stringify({ name, prompt, cwd, enabled: true, frequency: 'daily', time: '09:00', timezone: 'Asia/Hong_Kong', executionMode, notifications, notifyOn: 'always' }) }))
    } catch (caught) { setError(caught.message) }
    finally { setSaving(false) }
  }
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="modal" onSubmit={submit}><div className="card-head"><div><h2>{t('新建定时任务')}</h2><p>{t('创建后可继续设置运行时间和通知渠道。')}</p></div><button type="button" className="icon-button" aria-label={t('关闭对话框')} onClick={onClose}><X size={17} /></button></div><label className="field-label">{t('任务名称')}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={t('例如 每日代码巡检')} /></label><label className="field-label">Prompt<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={t('描述 Agent 每次需要完成的工作')} /></label><ScheduleWorkspaceField value={cwd} name={name} onChange={setCwd} /><ScheduleExecutionModeField value={executionMode} onChange={setExecutionMode} />{error && <div className="config-error"><AlertTriangle size={13} />{error}</div>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>{t('取消')}</button><button className="button primary" disabled={saving || !name.trim() || !prompt.trim()}>{saving ? <RefreshCw className="spin" size={14} /> : <Plus size={14} />}{t(saving ? '创建中…' : '创建任务')}</button></div></form></div>
}
