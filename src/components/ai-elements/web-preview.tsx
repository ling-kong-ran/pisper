...
export const WebPreview = ({
  ...
  return (
    <WebPreviewContext.Provider value={contextValue}>
      <div
        className={cn('flex size-full min-h-0 flex-col rounded-lg border bg-card', className)}
        {...props}
      >
        {children}
      </div>
...
export const WebPreviewNavigation = ({
  ...
  <div className={cn('flex shrink-0 items-center gap-1 border-b p-2', className)} {...props}>
    {children}
  </div>
)
...
export const WebPreviewUrl = ({ value, onChange, onKeyDown, ...props }: WebPreviewUrlProps) => {
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (event.key === 'Enter') setUrl((event.target as HTMLInputElement).value)
      onKeyDown?.(event)
    },
    [onKeyDown, setUrl],
  )
...
export const WebPreviewConsole = ({
  ...
  return (
    ...
    >
      ...
      <CollapsibleContent className="px-4 pb-4 outline-none data-[state=closed]:animate-out data-[state=open]:animate-in">
