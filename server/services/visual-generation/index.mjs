...
function noModelsError(kind) {
  return new Error(`没有已配置并启用的${kind === 'video' ? '视频' : '图像'}生成模型。请先在配置页添加视觉模型。`)
}
  ...
  async generate(request, options = {}) {
    ...
    for (const [index, model] of models.entries()) {
      ...
      try {
        ...
        options.onProgress?.(`${model.providerName} / ${model.name} 当前不可用，尝试 ${nextModel.providerName} / ${nextModel.name}…`)
      }
    }
