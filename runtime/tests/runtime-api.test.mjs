...
test('Extension APIs forward session scope, package ids, and Pi setting operations', async () => {
  const calls = []
  const runtime = {
    async getExtensionsDashboard(sessionId) {
      calls.push(['dashboard', sessionId])
      return { cwd: '/project', extensions: [] }
    },
    async installExtension(input, sessionId) {
      calls.push(['install', input.source, input.scope, sessionId])
      return { extensions: [] }
    },
    ...
    async updateExtension(id, enabled, sessionId) {
      calls.push(['toggle', id, enabled, sessionId])
      return { extensions: [] }
    },
    async updateExtensionPackage(id, sessionId) {
      calls.push(['update-package', id, sessionId])
      return { extensions: [] }
    },
    async removeExtensionPackage(id, sessionId) {
      calls.push(['remove-package', id, sessionId])
      return { extensions: [] }
    },
  }
  const handler = createApiHandler(runtime)
  ...
  await handler(
    request('GET'),
    dashboardResponse,
    new URL(`http://localhost/api/extensions${sessionQuery}`),
  )
  assert.equal(JSON.parse(dashboardResponse.body).cwd, '/project')
  await handler(
    request('POST', { source: 'npm:@fixture/extension', scope: 'project' }),
    response(),
    new URL(`http://localhost/api/extensions/install${sessionQuery}`),
  )
  await handler(
    request('POST', {}),
    response(),
    new URL(`http://localhost/api/extensions/reload${sessionQuery}`),
  )
  await handler(
    request('PATCH', { enabled: false }),
    response(),
    new URL(`http://localhost/api/extensions/ext-1${sessionQuery}`),
  )
  await handler(
    request('POST', {}),
    response(),
    new URL(`http://localhost/api/extensions/packages/pkg-1/update${sessionQuery}`),
  )
  await handler(
    request('DELETE'),
    response(),
    new URL(`http://localhost/api/extensions/packages/pkg-1${sessionQuery}`),
  )

  assert.deepEqual(calls, [
    ['dashboard', 'session 1'],
    ['install', 'npm:@fixture/extension', 'project', 'session 1'],
    ['reload', 'session 1'],
    ['toggle', 'ext-1', false, 'session 1'],
    ['update-package', 'pkg-1', 'session 1'],
    ['remove-package', 'pkg-1', 'session 1'],
  ])
})

test('Extension API rejects invalid scopes and enable payloads before runtime calls', async () => {
  const runtime = {
    async installExtension() {
      assert.fail('runtime install should not be called')
    },
    async updateExtension() {
      assert.fail('runtime toggle should not be called')
    },
  }
  const handler = createApiHandler(runtime)
  const invalidScope = response()
  await handler(
    request('POST', { source: './fixture', scope: 'workspace' }),
    invalidScope,
    new URL('http://localhost/api/extensions/install'),
  )
  assert.equal(invalidScope.status, 400)

  const invalidToggle = response()
  await handler(
    request('PATCH', { enabled: 'yes' }),
    invalidToggle,
    new URL('http://localhost/api/extensions/ext-1'),
  )
