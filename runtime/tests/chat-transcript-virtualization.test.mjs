...
test('virtualization source owns only message rows and preserves stable render boundaries', async () => {
  ...
  assert.match(autoScrollSource, /if \(!pinnedToBottomRef\.current\) return/)
  assert.match(autoScrollSource, /programmaticScrollRef\.current = true/)
  assert.match(autoScrollSource, /scheduleScrollToBottom\(\)/)
