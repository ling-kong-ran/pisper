...
test('Web pet service installs validated resources and publishes Agent state', async () => {
  ...
  try {
    ...
    status = service.remove('boba')
    assert.equal(status.enabled, false)
    assert.equal(status.running, false)
    assert.equal(status.selectedSlug, '')
    assert.deepEqual(status.installed, [])
    assert.equal(service.sprite('boba'), null)
    assert.throws(() => service.remove('boba'), /只能删除由 Pisper 安装的宠物/)
  } finally {
    service.dispose()
...
test('Tauri pet and tray menus preserve desktop pet controls', async () => {
  ...
  assert.match(petShell, /pet_window_is_visible_on_screen/)
  assert.match(petShell, /place_pet_window/)
  assert.match(petShell, /lifecycle: tokio::sync::Mutex/)
