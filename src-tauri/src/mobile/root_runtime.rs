...
pub(super) fn android_copy_asset(name: &str, target: &Path) -> Result<(), String> {
    ...
    with_android_env(|env, context| {
        ...
        let stream = env
            ...
            .and_then(|value| value.l())
            .map_err(|error| format!("无法打开完整 Runtime Asset：{error}"))?;
        let array = env
            .new_byte_array(64 * 1024)
            .map_err(|error| format!("无法分配 Asset 缓冲区：{error}"))?;
        let file = std::fs::File::create(target)
            .map_err(|error| format!("无法创建完整 Runtime 临时文件：{error}"))?;
        ...
        loop {
            let read = env
                ...
                .map_err(|error| format!("无法读取完整 Runtime Asset：{error}"))?;
            if read < 0 {
                break;
            }
            if read == 0 {
            ...
            let bytes = env
                ...
                .map_err(|error| format!("无法转换完整 Runtime Asset：{error}"))?;
            writer
                .write_all(&bytes[..read as usize])
                .map_err(|error| format!("无法写入完整 Runtime Asset：{error}"))?;
        }
        writer
            .flush()
            .map_err(|error| format!("无法刷新完整 Runtime Asset：{error}"))?;
        env.call_method(&stream, "close", "()V", &[])
            .map_err(|error| format!("无法关闭完整 Runtime Asset：{error}"))?;
        Ok(())
    })
}
