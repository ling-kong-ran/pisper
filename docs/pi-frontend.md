...
推送后核验 GitHub API 确认仓库默认分支为 `release`，不存在 `main`。同步修正源码更新检查与赞助配置请求使用的共享分支常量，以及更新页面的缺省标签；否则会请求 `main` 返回 404。源码更新只比较 release 基线，不自动覆盖独立前端；原生安装器、桌面更新通道和安装逻辑未修改。
