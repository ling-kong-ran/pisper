...
const SCRIPT_COMPONENTS = new Map([
  ...
  ['scripts/patch-pi-mobile-compat.mjs', ['desktop', 'runtime']],
  ['scripts/patch-pi-offline-compat.mjs', ['desktop', 'runtime']],
  ['scripts/search-tool-resources.json', ['desktop', 'runtime']],
  ['scripts/stage-search-tools.mjs', ['desktop', 'runtime']],
  ['scripts/stage-runtime-closure.mjs', ['desktop', 'runtime']],
...
export function releaseComponentsForPath(input) {
  ...
  // App 独占路径不参与其他组件检测；共享 Rust 路径仍归 Desktop。
  if (isAppExclusivePath(path)) return []
